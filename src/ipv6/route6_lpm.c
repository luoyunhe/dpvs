...
static inline int rt6_find_free_array_idx(nsid_t nsid, uint32_t *idx)
{
    ...
    if (this_rt6_array[nsid]->num >= g_rt6_array_size)
        return EDPVS_NOROOM;
    for (ii = (this_rt6_array[nsid]->cursor+1) % g_rt6_array_size;
...
static struct route6 *rt6_lpm_lookup(nsid_t nsid, const struct in6_addr *addr)
{
    uint32_t idx;
    struct route6 *rt6;

    if (rte_lpm6_lookup(this_lpm6_struct[nsid], (uint8_t*)addr, &idx) != 0)
        return this_rt6_default[nsid];

    assert(idx >= 0 && idx < g_rt6_array_size);
    rt6 = this_rt6_array[nsid]->entries[idx];
...
static struct route6 *rt6_lpm_input(nsid_t nsid, const struct rte_mbuf *mbuf, struct flow6 *fl6)
{
    ...
    rt6 = rt6_lpm_lookup(nsid, &fl6->fl6_daddr);
    if (!rt6)
        return NULL;
...
static int rt6_lpm_add_lcore(const struct dp_vs_route6_conf *rt6_cfg)
{
    ...
    ret = rt6_find_free_array_idx(nsid, &idx);
    ...
    ret = rte_lpm6_add(this_lpm6_struct[nsid], (uint8_t*)&entry->rt6_dst.addr,
    ...
    entry->arr_idx = idx;
    this_rt6_array[nsid]->num++;
    this_rt6_array[nsid]->cursor = idx;
    this_rt6_array[nsid]->entries[idx] = entry;
    hashkey = rt6_hash_key(&entry->rt6_dst);
    list_add_tail(&entry->hnode, &this_rt6_hash[nsid][hashkey]);
    ...
    RTE_LOG(DEBUG, RT6, "[%d] %s(%s via dev %s)->rt6_hash[%d]:rt6_array[%d] OK!"
            ...
            rt6_cfg->ifname, hashkey, idx, this_rt6_array[nsid]->num);
#endif
    return EDPVS_OK;
...
static int rt6_lpm_del_lcore(const struct dp_vs_route6_conf *rt6_cfg)
{
    ...
    list_for_each_entry_safe(entry, next, &this_rt6_hash[nsid][hashkey], hnode) {
        if (entry->rt6_dst.plen == rt6_cfg->dst.plen &&
            ...
            RTE_LOG(DEBUG, RT6, "[%d] %s(%s via dev %s)->rt6_hash[%d]:rt6_array[%d] OK!"
                    " %d routes left.\n", rte_lcore_id(), __func__, buf,
                    rt6_cfg->ifname, hashkey, entry->arr_idx, this_rt6_array[nsid]->num-1);
#endif
            list_del(&entry->hnode);
            this_rt6_array[nsid]->entries[entry->arr_idx] = NULL;
            this_rt6_array[nsid]->num--;
            route6_free(entry);
            /* no further search */
            break;
